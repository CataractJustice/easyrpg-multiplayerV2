...
void Transition::SetAttributesTransitions() {
	...
	case TransitionZoomOut:
		if (scene != nullptr && scene->type == Scene::Map) {
			zoom_position[0] = std::max(0, std::min(Main_Data::game_player->GetScreenX(), SCREEN_TARGET_WIDTH));
			zoom_position[1] = std::max(0, std::min(Main_Data::game_player->GetScreenY() - 8, SCREEN_TARGET_HEIGHT));
		}
		else {
