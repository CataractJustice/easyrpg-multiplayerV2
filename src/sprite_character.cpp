...
#include "multiplayer/packets/spritebitmappacket.h"
...
void Sprite_Character::OnCharSpriteReady(FileRequestResult*) {
	auto bitmap = Cache::Charset(character_name);
	SetBitmap(bitmap);
	...
	//BitmapRef crop = std::make_shared<Bitmap>(rect.width, rect.height, true);
	//crop->Blit(0, 0, *bitmap, rect, Opacity::Opaque());
	//if(character->GetType() == Game_Character::Player) sendPacket(OSpriteBitmapPacket(crop));
