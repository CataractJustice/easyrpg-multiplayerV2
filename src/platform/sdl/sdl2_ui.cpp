...
void Sdl2Ui::RequestVideoMode(int width, int height, int zoom, bool fullscreen, bool vsync) {
	...
	current_display_mode.width = width + CHAT_WIDTH;
	current_display_mode.height = height;
	current_display_mode.bpp = 32;
	current_display_mode.zoom = 2;//zoom;
...
bool Sdl2Ui::RefreshDisplayMode() {
	...
	if (!sdl_window) {
		...
		sdl_texture_game = SDL_CreateTexture(sdl_renderer,
			...
			(current_display_mode.width) * current_display_mode.zoom, current_display_mode.height * current_display_mode.zoom);
	...
	if (!main_surface) {
		...
		display_surface = Bitmap::Create((SCREEN_TARGET_WIDTH + CHAT_WIDTH)* current_display_mode.zoom, (SCREEN_TARGET_HEIGHT) * current_display_mode.zoom, Color(0, 0, 0, 255));
	}
...
void Sdl2Ui::UpdateDisplay() {
	GetFullresSurface()->Blit2x(GetFullresSurface()->GetRect(), *GetDisplaySurface(), GetDisplaySurface()->GetRect());
	FullresDrawPass(GetFullresSurface(), current_display_mode.zoom);
	// SDL_UpdateTexture was found to be faster than SDL_LockTexture / SDL_UnlockTexture.
	SDL_UpdateTexture(sdl_texture_game, nullptr, display_surface->pixels(), display_surface->pitch());
	...
	if (window.size_changed && window.width > 0 && window.height > 0) {
		...
		constexpr float want_aspect = (float)(SCREEN_TARGET_WIDTH + CHAT_WIDTH) / SCREEN_TARGET_HEIGHT;
		float real_aspect = width_float / height_float;

		if (scaling_mode == ScalingMode::Integer) {
			// Integer division on purpose
			if (want_aspect > real_aspect) {
				window.scale = static_cast<float>(window.width / (SCREEN_TARGET_WIDTH + CHAT_WIDTH));
			} else {
				window.scale = static_cast<float>(window.height / SCREEN_TARGET_HEIGHT);
			}

			viewport.w = static_cast<int>(ceilf((SCREEN_TARGET_WIDTH + CHAT_WIDTH) * window.scale));
			...
			window.scale = width_float / (SCREEN_TARGET_WIDTH + CHAT_WIDTH);
			viewport.x = 0;
		...
		if (scaling_mode == ScalingMode::Bilinear && window.scale > 0.f) {
			...
			sdl_texture_scaled = SDL_CreateTexture(sdl_renderer, texture_format, SDL_TEXTUREACCESS_TARGET,
			   static_cast<int>(ceilf(window.scale)) * (SCREEN_TARGET_WIDTH + CHAT_WIDTH), static_cast<int>(ceilf(window.scale)) * SCREEN_TARGET_HEIGHT);
